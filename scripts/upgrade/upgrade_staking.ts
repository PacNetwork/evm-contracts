...
const UpgradeContract = "PacUSDStaking"; //the new contract
/**
...
async function main() {
  ...
  if (!contractAddress) {
    throw new Error("[Error] Missing environment variable: STAKING_ADDRESS");
  }
  ...
  //call safe upgrade
  await safeUpgrade(proxyContract, newImplementation.target.toString(), initData);
}
