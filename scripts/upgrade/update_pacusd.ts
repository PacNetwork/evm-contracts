...
const UpdateContract = "PacUSD";
async function main() {
  const contractAddress = process.env.PACUSD_ADDRESS || "";
  ...
  console.log(`[Info] PacUSD Contract: ${contractAddress}`);
  ...
  try {
    ...
    // Encode update transaction data using UUPS proxy method
